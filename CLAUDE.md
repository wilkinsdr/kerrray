...
- Ray source classes provide different photon injection geometries:
  ...
  - `ImagePlane` — 2D observing plane, uniformly (linearly) spaced in `(x, y)`. Has a virtual destructor and a small virtual pixel-grid interface — `get_Nx()`/`get_Ny()`, `pixel_x(ix)`/`pixel_y(iy)`/`pixel_dx(ix)`/`pixel_dy(iy)` (pixel-center convention, `x0+(ix+0.5)*dx`, distinct from the older `ray_x`/`ray_y`/`get_x_index`/`get_y_index` "grid point" convention used by `CausticBundle`), and `pixel_weight(ix,iy)` (fixed at 1, matching every `RayTransfer` consumer's unweighted-sum convention over this uniform grid) — plus a virtual `init_ray`, added so `RayTransfer` (`src/ray_transfer/ray_transfer.h`) can query any concrete plane's pixel grid and initialize rays uniformly through a `const ImagePlane<T>&` reference, with no separate adapter object.
  - `LogImagePlane` (`src/raytracer/log_imageplane.h/.cpp`) — `ImagePlane` variant with a logarithmically-spaced `(x, y)` grid on each axis (mirrored about zero, with a linear zone spanning the origin so the grid tiles the plane with no gap — "symlog"), for resolving the optical axis (shadow/photon ring) finely while covering a wide field of view cheaply. Derives from `ImagePlane<T>` and reuses its `init_ray` as-is (public, const, grid-independent), adding only a guard for the one point `ImagePlane`'s own grid never reaches: exactly `x=y=0`, where `ImagePlane::init_ray`'s `asin(y/b)` is `0/0`. Since pixels are not equal-area, it overrides `get_Nx()`/`get_Ny()`/`pixel_x()`/`pixel_y()`/`pixel_dx()`/`pixel_dy()`/`pixel_weight()`/`init_ray` with its own log-spaced grid and per-cell area — selectable via `grid_type = log` in `ray_transfer_disc_wind`/`ray_transfer_disc_surface`/`ray_transfer_kerr_vs_flat`, just by constructing a `LogImagePlane` and passing it to `RayTransfer`'s single constructor like any other `ImagePlane`; `imageplane_disc_image`/`CausticBundle` still use `ImagePlane` only. Test: `log_imageplane_test`.
