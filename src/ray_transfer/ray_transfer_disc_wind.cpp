...
int main(int argc, char** argv)
{
    ...
    // view cheaply. See src/raytracer/log_imageplane.h and the pixel-grid query methods in imageplane.h.
    ...
    // log_x_min/log_x_max/log_Nx/log_Nlinx and the y-equivalents). Held by one polymorphic
    // unique_ptr<ImagePlane<double>> -- safe for either concrete type, since ImagePlane now has a virtual
    // destructor (imageplane.h) -- rather than two separately-typed unique_ptrs. RayTransfer takes just
    // this one plane object: its own get_Nx()/get_Ny()/pixel_x()/pixel_y()/pixel_weight() (virtual,
    // imageplane.h) supply the pixel grid, so there is no separate grid parameter/object to keep in sync.
